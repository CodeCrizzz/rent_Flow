    ...
    return (
        ...
        >
            ...
            >
                ...
                <p className="text-cyan-100/60 text-xs font-medium transform translate-z-8">{desc}</p>
            </div>
        </div>
    );
};
    ...
    return (
        <div ref={containerRef} onMouseMove={handleMouseMove} className="min-h-screen w-full bg-[#020617] text-white selection:bg-cyan-500/30 overflow-hidden relative font-sans flex flex-col group/container">
            <style>{`
                ...
                @keyframes breathe { 0%, 100% { transform: scale(1); opacity: 0.8; box-shadow: 0 0 30px rgba(6,182,212,0.15); } 50% { transform: scale(1.05); opacity: 1; box-shadow: 0 0 80px rgba(6,182,212,0.5); } }
            ...
            <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden transition-opacity duration-[2000ms] ease-out opacity-0 group-hover/container:opacity-100">
                ...
                {/* 1. Intense Amber Spotlight (Expanded & Brighter) */}
            ...
            <main className={`relative z-10 flex-1 flex flex-col items-center justify-center px-6 text-center transition-all duration-[1200ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${isEntering ? 'opacity-0 scale-90 blur-3xl' : 'opacity-100 scale-100 blur-0'}`}>
                ...
                <h1 className="text-6xl sm:text-7xl lg:text-9xl font-black tracking-tighter mb-6 leading-[0.85]">
                    Manage <br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-blue-400 to-indigo-400 drop-shadow-[0_0_50px_rgba(6,182,212,0.3)]">Every Unit.</span>
                </h1>

                <p className="text-slate-300 text-sm md:text-base max-w-md mb-10 leading-relaxed font-bold">
