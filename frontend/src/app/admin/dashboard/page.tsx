    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
                ...
                {/* Card 1: Rooms (Indigo) */}
                <div className="bg-white p-8 border border-slate-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-4xl hover:-translate-y-1 hover:shadow-[0_8px_30px_rgb(79,70,229,0.08)] transition-all duration-300 flex flex-col justify-between group">
                    <div className="flex justify-between items-start mb-8">
                        <div className="w-14 h-14 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white group-hover:scale-110 transition-all duration-300 shadow-sm border border-indigo-100 group-hover:border-transparent group-hover:shadow-indigo-500/30">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg">Total Metrics</span>
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-slate-500 mb-1">Registered Rooms</h3>
                        <p className="text-5xl font-black text-slate-900 tracking-tight">{stats.totalRooms}</p>
                    </div>
                </div>

                {/* Card 2: Tenants (Emerald) */}
                <div className="bg-white p-8 border border-slate-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-4xl hover:-translate-y-1 hover:shadow-[0_8px_30px_rgb(16,185,129,0.08)] transition-all duration-300 flex flex-col justify-between group">
                    <div className="flex justify-between items-start mb-8">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg">Active System</span>
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-slate-500 mb-1">Current Tenants</h3>
                        <p className="text-5xl font-black text-slate-900 tracking-tight">{stats.activeTenants}</p>
                    </div>
                </div>

                {/* Card 3: Payments (Rose / Ambar pending) */}
                <div className="bg-white p-8 border border-slate-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-4xl hover:-translate-y-1 hover:shadow-[0_8px_30px_rgb(244,63,94,0.08)] transition-all duration-300 flex flex-col justify-between group">
                    <div className="flex justify-between items-start mb-8">
                        <div className="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-600 group-hover:bg-rose-500 group-hover:text-white group-hover:scale-110 transition-all duration-300 shadow-sm border border-rose-100 group-hover:border-transparent group-hover:shadow-rose-500/30">
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        </div>
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg">Outstanding</span>
                    </div>
                    <div>
                        <h3 className="text-sm font-bold text-slate-500 mb-1">Total Dues Pending</h3>
