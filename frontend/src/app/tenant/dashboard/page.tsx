...
export default function TenantDashboard() {
    ...
    return (
        ...
        <div className="w-full h-[100dvh] text-neutral-900 dark:text-neutral-100 font-sans flex flex-col bg-transparent overflow-hidden relative [ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            {/* Main Wrapper: h-full and flex-col to lock layout to exact screen height */}
            <div className="max-w-6xl mx-auto w-full h-full flex flex-col space-y-4 sm:space-y-6 pt-4 sm:pt-6 px-3 sm:px-6 lg:px-8 pb-4 relative z-10">
                ...
                <div className={`shrink-0 grid grid-cols-12 gap-3 sm:gap-6 transition-all duration-700 ease-out delay-150 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                    ...
                    <div className="col-span-7 lg:col-span-8 relative rounded-2xl sm:rounded-4xl bg-white/60 dark:bg-[#121212]/60 backdrop-blur-2xl shadow-xl shadow-indigo-500/5 border border-white/40 dark:border-white/10 overflow-hidden flex flex-col justify-between p-4 sm:p-8 lg:p-10">
                        ...
                        <div className="relative z-10">
                            <h3 className="text-[8px] sm:text-xs font-bold text-neutral-400 dark:text-neutral-500 uppercase tracking-widest mb-1 sm:mb-2">Total Outstanding</h3>
                            <div className="flex items-baseline gap-0.5 sm:gap-1">
                                <span className="text-2xl sm:text-5xl lg:text-6xl font-black tracking-tighter text-neutral-900 dark:text-white font-mono break-all">
                                    ₱{displayBalance.toLocaleString()}
                                </span>
                                <span className="text-sm sm:text-2xl lg:text-3xl font-medium text-neutral-400 dark:text-neutral-500">.00</span>
                            </div>
                        </div>

                        <div className="relative z-10 mt-3 sm:mt-12 flex flex-col sm:flex-row gap-3 sm:gap-6 items-start sm:items-center justify-between border-t border-neutral-200/50 dark:border-white/10 pt-3 sm:pt-8">
                            <p className="text-[8px] sm:text-sm font-medium text-neutral-500 dark:text-neutral-400 w-full sm:max-w-xs leading-tight sm:leading-relaxed">
                                Your next billing cycle generates on the 1st of the month.
                            </p>
                            
                            {payStatus ? (
                                <div className="w-full sm:w-auto px-3 sm:px-8 py-2 sm:py-3.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-center font-bold rounded-lg sm:rounded-2xl animate-in zoom-in duration-300 flex items-center justify-center gap-1.5 shadow-[0_0_20px_rgba(16,185,129,0.15)] text-[9px] sm:text-base">
                                    <svg className="w-3 h-3 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"></path></svg>
                                    {payStatus}
                                </div>
                            ) : (
                                <button 
                                    ...
                                    className={`w-full sm:w-auto px-3 sm:px-8 py-2 sm:py-3.5 font-bold rounded-lg sm:rounded-2xl transition-all duration-300 flex items-center justify-center gap-1.5 backdrop-blur-md text-[9px] sm:text-base ${
                                ...
                                >
                                    {isPaying && <div className="w-2.5 h-2.5 sm:w-4 sm:h-4 border-2 border-current/30 border-t-current rounded-full animate-spin"></div>}
                    ...
                    {/* SIDE CARDS */}
                    <div className="col-span-5 lg:col-span-4 flex flex-col gap-3 sm:gap-6">
                        
                        {/* Status Card */}
                        <div className="relative bg-white/60 dark:bg-[#121212]/60 p-3 sm:p-6 rounded-2xl sm:rounded-4xl border border-white/40 dark:border-white/10 backdrop-blur-xl shadow-xl shadow-indigo-500/5 flex-1 flex flex-col justify-center overflow-hidden">
                            <div className="absolute inset-0 glass-noise z-0"></div>
                            <div className="relative z-10 flex items-center justify-between mb-2 sm:mb-4">
                                <div className="p-1.5 sm:p-2.5 rounded-md sm:rounded-xl bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">
                                    <svg className="w-3 h-3 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                                </div>
                                <span className="text-[7px] sm:text-xs font-bold uppercase tracking-widest text-neutral-400">Unit Status</span>
                            </div>
                            <div className="relative z-10">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                                    <span className={`w-1.5 h-1.5 sm:w-2.5 sm:h-2.5 rounded-full shadow-[0_0_12px_rgba(249,115,22,0.8)] animate-pulse ${tenantData.status === 'Active' ? 'bg-emerald-500' : 'bg-orange-500'}`}></span>
                                    <p className="text-sm sm:text-2xl font-bold">{tenantData.status || 'Pending'}</p>
                                </div>
                                <p className="text-[7px] sm:text-sm font-medium text-neutral-500 dark:text-neutral-400 leading-tight">
                        ...
                        {/* Messages Card */}
                        <div className="relative bg-white/60 dark:bg-[#121212]/60 p-3 sm:p-6 rounded-2xl sm:rounded-4xl border border-white/40 dark:border-white/10 backdrop-blur-xl shadow-xl shadow-indigo-500/5 flex-1 flex flex-col justify-center hover:bg-white/80 dark:hover:bg-[#181818]/80 hover:border-indigo-200 dark:hover:border-indigo-500/30 cursor-pointer group transition-all duration-300 overflow-hidden">
                            <div className="absolute inset-0 glass-noise z-0"></div>
                            <div className="relative z-10 flex items-center justify-between mb-2 sm:mb-4">
                                <div className="p-1.5 sm:p-2.5 rounded-md sm:rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                                    <svg className="w-3 h-3 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
                                </div>
                                <span className="text-[7px] sm:text-xs font-bold uppercase tracking-widest text-neutral-400">Messages</span>
                            </div>
                            <div className="relative z-10">
                                <p className="text-sm sm:text-2xl font-bold mb-0.5 sm:mb-1">0 Unread</p>
                                <Link href="/tenant/chat" className="inline-flex items-center text-[8px] sm:text-sm font-bold text-indigo-600 dark:text-indigo-400 group-hover:text-indigo-700 dark:group-hover:text-indigo-300">
                                    Open Inbox <svg className="w-2.5 h-2.5 sm:w-4 sm:h-4 ml-1 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
                                </Link>
                            </div>
                        </div>
                    </div>
                ...
                {/* --- TRANSACTIONS SECTION (flex-1 so it takes remaining vertical space) --- */}
                    ...
                    <div className="relative z-10 flex-1 overflow-y-auto p-2 sm:p-4 [ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                        {tenantData.recentTransactions && tenantData.recentTransactions.length > 0 ? (
                                ...
                                {tenantData.recentTransactions.slice(0, 3).map((tx: any, i) => (
                                    <div key={i} className="group relative flex items-center justify-between p-2.5 sm:p-4 rounded-xl sm:rounded-2xl hover:bg-white/40 dark:hover:bg-white/5 transition-all duration-300 cursor-default gap-2">
                                        ...
                                        <div className="flex items-center gap-2.5 sm:gap-4 flex-1 min-w-0">
                                            <div className={`w-7 h-7 sm:w-10 sm:h-10 rounded-md sm:rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-sm ${
                                                ...
                                                {tx.type === 'payment' && <svg className="w-3.5 h-3.5 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>}
                                                {tx.type === 'charge' && <svg className="w-3.5 h-3.5 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[10px] sm:text-sm font-bold text-neutral-900 dark:text-neutral-100 truncate">{tx.description}</p>
                                                <p className="text-[8px] sm:text-xs font-medium text-neutral-500 dark:text-neutral-500 mt-0.5">{tx.date}</p>
                                            </div>
                                        </div>
                                        <div className="text-right shrink-0">
                                            <p className={`text-[10px] sm:text-base font-black tracking-tight font-mono ${tx.type === 'payment' ? 'text-emerald-600 dark:text-emerald-400' : 'text-neutral-900 dark:text-neutral-100'}`}>
                                                {tx.type === 'payment' ? '-' : ''}₱{tx.amount.toLocaleString()}
                                            </p>
                                            <p className={`text-[7px] sm:text-[10px] font-bold uppercase tracking-widest mt-0.5 sm:mt-1 ${
                            ...
                            <div className="flex flex-col items-center justify-center py-10 h-full px-4 text-center">
                                <div className="w-12 h-12 sm:w-20 sm:h-20 bg-white/50 dark:bg-white/5 rounded-2xl sm:rounded-4xl flex items-center justify-center text-neutral-400 dark:text-neutral-500 mx-auto mb-3 sm:mb-6 shadow-inner border border-neutral-200/50 dark:border-white/5 backdrop-blur-md">
                                    <svg className="w-6 h-6 sm:w-10 sm:h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                </div>
                                <h3 className="text-xs sm:text-lg font-bold text-neutral-900 dark:text-white">No Transactions Yet</h3>
                                <p className="text-[9px] sm:text-sm font-medium text-neutral-500 mt-1 sm:mt-2 max-w-xs mx-auto leading-relaxed">Payments and invoices will appear here once they are processed.</p>
                            </div>
                        )}
                    </div>
                </div>
