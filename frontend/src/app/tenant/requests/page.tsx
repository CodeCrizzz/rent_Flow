...
export default function TenantMaintenance() {
    ...
    return (
        <div className="fixed inset-0 w-full h-full md:pl-64 lg:pl-72 text-neutral-900 dark:text-neutral-100 font-sans flex flex-col bg-transparent overflow-hidden">
            ...
            <motion.div initial="hidden" animate="visible" variants={containerVariants} className="max-w-[1400px] mx-auto w-full h-full flex flex-col min-h-0 gap-2 sm:gap-4 pt-6 px-3 sm:px-8 pb-3 sm:pb-4 relative z-10">
                ...
                <motion.div variants={itemVariants} className="shrink-0 relative rounded-xl sm:rounded-2xl bg-white/60 dark:bg-[#121212]/60 backdrop-blur-2xl shadow-xl shadow-indigo-500/5 border border-white/40 dark:border-white/10 p-2 sm:p-4 overflow-hidden">
                    ...
                    <div className="relative z-10 grid grid-cols-4 divide-x divide-neutral-200/50 dark:divide-white/10">
                        <div className="px-2 sm:px-6 text-center sm:text-left"><p className="text-[6px] sm:text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Total</p><p className="text-[10px] sm:text-2xl font-black font-mono">{summary.total}</p></div>
                        <div className="px-2 sm:px-6 text-center sm:text-left"><p className="text-[6px] sm:text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Pending</p><p className="text-[10px] sm:text-2xl font-black font-mono text-amber-500">{summary.pending}</p></div>
                        <div className="px-2 sm:px-6 text-center sm:text-left"><p className="text-[6px] sm:text-[10px] font-bold text-neutral-500 uppercase tracking-widest">In Progress</p><p className="text-[10px] sm:text-2xl font-black font-mono text-blue-500">{summary.progress}</p></div>
                        <div className="px-2 sm:px-6 text-center sm:text-left"><p className="text-[6px] sm:text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Resolved</p><p className="text-[10px] sm:text-2xl font-black font-mono text-emerald-500">{summary.resolved}</p></div>
                    </div>
                </motion.div>
                ...
                <div className="flex-1 min-h-0 grid grid-cols-12 gap-2 sm:gap-5">
                    ...
                    {/* NEW REQUEST FORM */}
                    <motion.div variants={itemVariants} className="col-span-4 relative rounded-xl sm:rounded-[2rem] bg-white/60 dark:bg-[#121212]/60 backdrop-blur-2xl shadow-xl shadow-indigo-500/5 border border-white/40 dark:border-white/10 overflow-hidden flex flex-col">
                        <div className="absolute inset-0 glass-noise z-0 pointer-events-none"></div>
                    ...
                    <motion.div variants={itemVariants} className="col-span-8 relative bg-white/60 dark:bg-[#121212]/60 rounded-xl sm:rounded-[2rem] border border-white/40 dark:border-white/10 backdrop-blur-2xl shadow-xl shadow-indigo-500/5 flex flex-col min-h-0 overflow-hidden">
                        <div className="absolute inset-0 glass-noise z-0 pointer-events-none"></div>

                        {/* Toolbar */}
                        <div className="relative z-10 shrink-0 p-2 sm:p-4 border-b border-neutral-200/50 dark:border-white/10 bg-white/40 dark:bg-[#121212]/40 backdrop-blur-md flex items-center justify-between gap-2 overflow-x-auto custom-scrollbar">
                            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                                <div className="relative w-20 sm:w-48">
                                    <svg className="absolute left-1.5 sm:left-2.5 top-1/2 -translate-y-1/2 w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                                    <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white dark:bg-black/20 border border-neutral-200 dark:border-white/10 rounded-md sm:rounded-xl pl-5 sm:pl-8 pr-2 py-1 sm:py-2 text-[6px] sm:text-xs outline-none focus:ring-2 focus:ring-indigo-500/50 text-neutral-900 dark:text-white" />
                                </div>
                                <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setCurrentPage(1); }} className="bg-white dark:bg-black/20 border border-neutral-200 dark:border-white/10 rounded-md sm:rounded-xl px-1 sm:px-2 py-1 sm:py-2 text-[6px] sm:text-xs outline-none cursor-pointer text-neutral-900 dark:text-white">
                                ...
                                <select value={filterCategory} onChange={(e) => { setFilterCategory(e.target.value); setCurrentPage(1); }} className="bg-white dark:bg-black/20 border border-neutral-200 dark:border-white/10 rounded-md sm:rounded-xl px-1 sm:px-2 py-1 sm:py-2 text-[6px] sm:text-xs outline-none cursor-pointer text-neutral-900 dark:text-white">
                            ...
                            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-white dark:bg-black/20 border border-neutral-200 dark:border-white/10 rounded-md sm:rounded-xl px-1 sm:px-2 py-1 sm:py-2 text-[6px] sm:text-xs outline-none cursor-pointer text-neutral-900 dark:text-white">
                        ...
                        <div className="relative z-10 flex-1 overflow-y-auto overflow-x-auto custom-scrollbar">
                            <table className="w-full text-left border-collapse min-w-[350px] sm:min-w-[700px]">
                                        ...
                                        paginatedRequests.map((r) => (
                                            <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="group hover:bg-white/40 dark:hover:bg-white/[0.02] transition-all duration-300">
                                                ...
                                                <td className="px-2 py-2 sm:px-4 sm:py-3 align-middle min-w-0">
                                                    <p className="text-[7px] sm:text-sm font-bold truncate max-w-[100px] sm:max-w-[200px] text-neutral-900 dark:text-white" title={r.title}>{r.title}</p>
                                                ...
                                                <td className="px-2 py-2 sm:px-4 sm:py-3 align-middle text-right">
                                                    <button onClick={() => setSelectedRequest(r)} className="p-1 sm:p-1.5 rounded bg-neutral-100 dark:bg-white/5 hover:text-indigo-600 transition-colors text-neutral-500" title="View Details">
                                ...
                                <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto custom-scrollbar">
                                    ...
                                    {selectedRequest.image_url && (
                                        <div>
                                            ...
                                            <img onClick={() => setViewImageUrl(selectedRequest.image_url || null)} src={selectedRequest.image_url} alt="Proof" className="w-20 sm:w-32 h-20 sm:h-32 object-cover rounded-xl cursor-zoom-in border border-neutral-200 dark:border-white/10 hover:opacity-80 transition-opacity" />                                        </div>
                                    )}
                                </div>
                    ...
                    <>
                        ...
                        <div className="fixed inset-0 flex items-center justify-center p-4 z-[201] pointer-events-none">
                            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative pointer-events-auto">
                                <button onClick={() => setViewImageUrl(null)} className="absolute -top-12 right-0 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white backdrop-blur-md transition-colors"><svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
