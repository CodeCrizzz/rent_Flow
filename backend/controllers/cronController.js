...
// @desc    Run automatic billing cron job
// @route   POST /api/cron/billing
// @access  Cron
const runBillingCron = async (req, res) => {
    const authHeader = req.headers.authorization;
    if (authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    console.log('Running automatic billing cron job via Vercel Cron...');
    try {
        ...
        if (tenants.length === 0) {
            ...
            return res.status(200).json({ message: 'No active tenants found for automatic billing.' });
        }
        ...
        res.status(200).json({ message: 'Automatic billing cron job completed successfully.' });
    } catch (error) {
        console.error('Error running automatic billing cron job:', error);
        res.status(500).json({ message: 'Error running automatic billing cron job' });
    }
};
